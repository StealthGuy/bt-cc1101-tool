...
{ 
  ...
  // identification of the command & actions
    ...
    // Handling SETMODULATION command 
        ...
        SerialBT.print(F("\r\nModulation: "));
        if (setting == 0) { SerialBT.print(F("2-FSK")); }
        else if (setting == 1) { SerialBT.print(F("GFSK")); }
        else if (setting == 2) { SerialBT.print(F("ASK/OOK")); }
        else if (setting == 3) { SerialBT.print(F("4-FSK")); }
        else if (setting == 4) { SerialBT.print(F("MSK")); };  
        SerialBT.print(F(" \r\n"));
    ...
    // Handling SETMHZ command 
        ...
        SerialBT.print(F("\r\nFrequency: "));
        SerialBT.print(settingf1);
        SerialBT.print(F(" MHz\r\n"));
    ...
    // Handling SETSYNCMODE command         
        ...
        SerialBT.print(F("\r\nSynchronization: "));
        if (setting == 0) { SerialBT.print(F("No preamble")); }
        else if (setting == 1) { SerialBT.print(F("16 sync bits")); }
        else if (setting == 2) { SerialBT.print(F("16/16 sync bits")); }
        else if (setting == 3) { SerialBT.print(F("30/32 sync bits")); }
        else if (setting == 4) { SerialBT.print(F("No preamble/sync, carrier-sense")); }
        else if (setting == 5) { SerialBT.print(F("15/16 + carrier-sense")); }
        else if (setting == 6) { SerialBT.print(F("16/16 + carrier-sense")); }
        else if (setting == 7) { SerialBT.print(F("30/32 + carrier-sense")); };
        SerialBT.print(F("\r\n"));  
    ...
    // Handling SETSYNCWORD command         
        ...
        SerialBT.print(F("\r\nSynchronization:\r\n"));
        SerialBT.print(F("high = "));
        SerialBT.print(setting);
        SerialBT.print(F("\r\nlow = "));
        SerialBT.print(setting2);
        SerialBT.print(F("\r\n"));  
    ...
    // Handling SETADRCHK command         
        ...
        SerialBT.print(F("\r\nAddress checking:"));
        if (setting == 0) { SerialBT.print(F("No adr chk")); }
        else if (setting == 1) { SerialBT.print(F("Adr chk, no bcast")); }
        else if (setting == 2) { SerialBT.print(F("Adr chk and 0 bcast")); }
        else if (setting == 3) { SerialBT.print(F("Adr chk and 0 and FF bcast")); };
        SerialBT.print(F("\r\n"));  
    ...
    // Handling SETWHITEDATA command         
        ...
        SerialBT.print(F("\r\nWhitening "));
        if (setting == 0) { SerialBT.print(F("OFF")); }
        else if (setting == 1) { SerialBT.print(F("ON")); }
        SerialBT.print(F("\r\n"));  
    ...
    // Handling SETPKTFORMAT command         
        ...
        SerialBT.print(F("\r\nPacket format: "));
        if (setting == 0) { SerialBT.print(F("Normal mode")); }
        else if (setting == 1) { SerialBT.print(F("Synchronous serial mode")); }
        else if (setting == 2) { SerialBT.print(F("Random TX mode")); }
        else if (setting == 3) { SerialBT.print(F("Asynchronous serial mode")); };
        SerialBT.print(F("\r\n"));  
    ...
    // Handling SETLENGTHCONFIG command         
        ...
        SerialBT.print(F("\r\nPkt length mode: "));
        if (setting == 0) { SerialBT.print(F("Fixed")); }
        else if (setting == 1) { SerialBT.print(F("Variable")); }
        else if (setting == 2) { SerialBT.print(F("Infinite")); }
        else if (setting == 3) { SerialBT.print(F("Reserved")); };
        SerialBT.print(F("\r\n"));  
    ...
    // Handling SETCRC command         
        ...
        SerialBT.print(F("\r\nCRC checking: "));
        if (setting == 0) { SerialBT.print(F("Disabled")); }
        else if (setting == 1) { SerialBT.print(F("Enabled")); };
        SerialBT.print(F("\r\n")); 
    ...
    // Handling SETDCFILTEROFF command         
        ...
        SerialBT.print(F("\r\nDC filter: "));
        if (setting == 0) { SerialBT.print(F("Enabled")); }
        else if (setting == 1) { SerialBT.print(F("Disabled")); };
        SerialBT.print(F("\r\n")); 
    ...
    // Handling SETPRE command         
        ...
        SerialBT.print(setting);
        SerialBT.print(F(" means 0 = 2 bytes, 1 = 3b, 2 = 4b, 3 = 6b, 4 = 8b, 5 = 12b, 6 = 16b, 7 = 24 bytes\r\n")); 
        SerialBT.print(F("\r\n")); 
    ...
    // Handling GETRSSI command         
        ...
        //Rssi Level in dBm
        SerialBT.print(F("Rssi: "));
        SerialBT.println(ELECHOUSE_cc1101.getRssi());
        //Link Quality Indicator
        SerialBT.print(F(" LQI: "));
        SerialBT.println(ELECHOUSE_cc1101.getLqi());        
        SerialBT.print(F("\r\n")); 
    ...
    // Handling SCAN command - frequency scanner by Little S@tan !
        ...
        SerialBT.print(settingf1);
        SerialBT.print(F(" MHz to "));
        SerialBT.print(settingf2);
        SerialBT.print(F(" MHz, press any key for stop or wait...\r\n"));  
                   ...
                   if (mark_rssi>-75)
                    {
                      ...
                      if (fr == compare_freq)
                          {
                            SerialBT.print(F("\r\nSignal found at  "));
                            SerialBT.print(F("Freq: "));
                            SerialBT.print(mark_freq);
                            SerialBT.print(F(" Rssi: "));
                            SerialBT.println(mark_rssi);
                            mark_rssi=-100;
                            compare_freq = 0;
                            mark_freq = 0;
                          }
    ...
    // Handling RX command         
        ...
        if (receivingmode == 1) {
          receivingmode = 0;
          SerialBT.print(F("Disabled")); }
        else if (receivingmode == 0)
               { ELECHOUSE_cc1101.SetRx();
                 SerialBT.print(F("Enabled")); 
                 receivingmode = 1;
                 jammingmode = 0; 
                 recordingmode = 0;
               };
    ...
    // handling BRUTE command
        ...
        else { SerialBT.print(F("Wrong parameters.\r\n")); };
    ...
    // Handling TX command         
        ...
        { 
                ...
                SerialBT.print(F("Sent frame: "));
                SerialBT.print((char *)textbuffer);
                SerialBT.print(F("\r\n")); }
         else { SerialBT.print(F("Wrong parameters.\r\n")); };
    ...
    // handling RECRAW command
        ...
        //while (!SerialBT.available() ||  (digitalRead(gdo0) == LOW) ); 
   ...
   // handling RXRAW command - sniffer
           ...
           {        // first convert to hex numbers
                    ...
                    for (setting = 0; setting < 32 ; setting++)
                        ...
                        switch( setting2 )
                              {
                              case '0':
                              SerialBT.print(F("____"));
                              break;
   
                              case '1':
                              SerialBT.print(F("___-"));
                              break;
   
                              case '2':
                              SerialBT.print(F("__-_"));
                              break;
                              ...
                              case '4':
                              SerialBT.print(F("_-__"));
                              break;

                              case '5':
                              SerialBT.print(F("_-_-"));
                              break;
        ...
        { 
                ...
                if (( bigrecordingbufferpos + len + 1) < RECORDINGBUFFERSIZE) 
                     { // put info about number of bytes
                      ...
                      SerialBT.print(F("\r\nAdded frame number "));
                      SerialBT.print(framesinbigrecordingbuffer);
                      SerialBT.print(F("\r\n"));                  
                    }   
               else                  
                   {   
    ...
    // command 'init' initializes board with default settings
        ...
        cc1101initialize();
        // give feedback
        SerialBT.print(F("CC1101 initialized\r\n"));
          
    } else {
        SerialBT.print(F("Error: Unknown command: "));
...
void setup() {
     ...
     Serial.begin(115200);
     SerialBT.begin("BlueRF");

    //Init EEPROM - for ESP32 based boards only
     EEPROM.begin(EPROMSIZE);     

     SerialBT.println(F("CC1101 terminal tool connected, use 'help' for list of commands...\n\r"));
     SerialBT.println(F("(C) Adam Loboda 2023\n\r  "));

     SerialBT.println();  // print CRLF
     ...
     cc1101initialize();
      ...
      SerialBT.println(F("cc1101 initialized. Connection OK\n\r"));
      } else {
      SerialBT.println(F("cc1101 connection error! check the wiring.\n\r"));
      };
  ...
  int i = 0;
    ...
    if (chatmode == 1) 
       { 
            ...
            while (SerialBT.available() and (i<(CCBUFFERSIZE-1)) ) 
             {
              // read single character from Serial port         
              ccsendingbuffer[i] = SerialBT.read();

              // also put it as ECHO back to serial port
              SerialBT.write(ccsendingbuffer[i]);
    ...
    // handling CLI commands processing
      ...
      {   
        ...
        if (data == '\b' || data == '\177') {  // BS and DEL
            if (length) {
                length--;
                if (do_echo) SerialBT.printf("\b \b");
            }
        }
        else if (data == '\r' || data == '\n' ) {
            if (do_echo) SerialBT.printf("\r\n");    // output CRLF
            buffer[length] = '\0';
            ...
            buffer[length++] = data;
            if (do_echo) SerialBT.write(data);
        }
       };  
            ...
            if ( ( chatmode == 1) && (len < CCBUFFERSIZE ) )
               {
                ...
                SerialBT.print((char *) ccreceivingbuffer);
            ...
            if ( ((recordingmode == 1) && (receivingmode == 0) )&& (len < CCBUFFERSIZE ) )
               { 
                ...
                if (( bigrecordingbufferpos + len + 1) < RECORDINGBUFFERSIZE) 
                    ...
                    SerialBT.print(F("Recording buffer full! Stopping..\r\nFrames stored: "));
                    SerialBT.print(framesinbigrecordingbuffer); 
                    SerialBT.print(F("\r\n"));
                    bigrecordingbufferpos = 0;
                    recordingmode = 0;
